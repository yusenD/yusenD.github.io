...
刚才用数组实现的是固定大小的   一个栈，当栈满的时候，就无法再往栈里面添加数据了。实现思路和在数组那里的类似，满了的时候，申请一个更大的数组，然后把原来的数组复制进去。
...
{% highlight java%}
    ...
    static Stack calSignStack = new Stack();
    static Stack numStack = new Stack();

    public static void main(String[] args){

        String str1 = "3+5*8-6";
        String str2 = "1+2*3/3*2/2+5";
    ...
    public static void countStr(String str){
        for(int i=0;i<str.length();i++){
            char c = str.charAt(i);
            if(isNum(c)) numStack.push(Integer.valueOf(str.substring(i,i+1)));
            else {
                if(shouldCount(isCountSign(c), (int) calSignStack.getTOP())){
                    float b = numStack.pop();
                    float a = numStack.pop();
                    float sum = count(a,b, calSignStack.pop());
                    numStack.push(sum);
                }
                calSignStack.push(isCountSign(c));
            }
        }
        while(!calSignStack.isEmpty()){
            float b = numStack.pop();
            float a = numStack.pop();
            float sum = count(a,b, calSignStack.pop());
            numStack.push(sum);
        }
        System.out.println("结果为：" + numStack.getTOP());
    }

    public static float count(float a,float b,float f){
        if(f==PLUS) return a+b;
        if(f==SUB) return a-b;
        if(f==MUL) return a*b;
        if(f==DIV) return a/b;
        return ERROR;
    }
    public static boolean shouldCount(int a,int b){
        if(a<=b) return true;
    ...
    public static int isCountSign(char str) {
        if(str == '+') return PLUS;
        if(str == '-') return SUB;
        if(str == '*') return MUL;
        if(str == '/') return DIV;
...
public class Bracket {
    ...
    private int SMALL_L = 1;
    private int SMALL_R = -1;
    ...
    public boolean judgement() {
        ...
        for (int i = 0; i < judgeStr.length(); i++) {
            int sign = turnToBracket(judgeStr.charAt(i));
            if (sign > 0) stack.push(sign);
            if (sign < 0) {
                if((stack.pop()+sign)!=0) return false;
            }
        }
    ...
    public int turnToBracket(char c) {
