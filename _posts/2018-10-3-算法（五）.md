...
如果知道环的长度n的话，可以设置一个fast指针，一个slow指针，fast先走n步，然后fast和slow走相同步长，相遇的节点就是环的入口节点。

或者利用set的特性，利用set来进行判断。
...
{% highlight java%}
    ...
    public void merge() {
        Link m = new Link();
        Link.Item cur1 = l1.getHead().next;
        Link.Item cur2 = l2.getHead().next;
